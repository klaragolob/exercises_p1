...
let rec height t = 
	match t with
	| Empty -> 0
	| Node(r, x, l) -> 1 + (max (height r) (height l))

let rec size t = 
	match t with 
	|Empty -> 0
	| Node(r, x, l) -> 1 + size r + size l
...
let rec follow directions t = 
	...
	| ([], Node(_, x, _)) ->  Some x 
	| (_, Empty) -> None 
	| (hd::tl, Node(l, _, r))  -> if hd = Left then follow tl l else follow tl r
...
let rec prune directions t =
	...
	| (_::_, Empty) -> None 
	| ([], _ )-> Some Empty
	| (direction::directions, Node(l, x, r)) -> 
		(match direction with 
		| Left ->
			let l_pruned = prune directions l in 
			(match l_pruned with
			|None -> None 
			|Some l_pruned -> Some(Node (l_pruned, x, r)))
		|Right ->
			(match prune directions r with
			|None -> None
