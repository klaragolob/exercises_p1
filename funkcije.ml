...
let mapi f l = 
	let rec mapi_aux f l acc index =
		match l with 
		| [] -> reverse acc
		| hd::tl -> mapi_aux f tl ((f index hd) ::acc) (index + 1)
	in 
	mapi_aux f l [] 0
...
let zip l1 l2 = 
	let rec zip_aux l1 l2 acc =
		match (l1, l2) with 
		| ([], []) -> reverse acc
		| (l1, []) -> failwith "Seznam razlicih dolzin."
		| ([], l2) -> failwith "Seznam razlicnih dolzin."
		| (hd1::tl1, hd2::tl2) -> zip_aux tl1 tl2 ((hd1, hd2)::acc) 
	in 
	zip_aux l1 l2 [] 
